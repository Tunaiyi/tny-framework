 ...
 * @author KGTny
 */
public class LocalAsyncObjectPool implements ObjectPool {
     ...
     * 异步持久化器f管理器
     */
    private SynchronizerHolder synchronizerHolder;

    public LocalAsyncObjectPool(SyncDBExecutor syncDBExecutor, SynchronizerHolder synchronizerHolder, final long recycleTime) {
    ...
    public LocalAsyncObjectPool(SyncDBExecutor syncDBExecutor, ReleaseStrategyFactory releaseStrategyFactory, SynchronizerHolder synchronizerHolder,
        ...
        this.recycleExecutor.schedule(new Runnable() {
            ...
            @Override
            public void run() {
                int size = LocalAsyncObjectPool.this.entityMap.size();
                int removeSize = 0;
                long releaseAt = System.currentTimeMillis();
                for (Entry<String, AsyncDBEntity> entry : LocalAsyncObjectPool.this.entityMap.entrySet()) {
                    try {
                        if (entry.getValue().release(releaseAt)) {
                            if (LocalAsyncObjectPool.this.entityMap.remove(entry.getKey(), entry.getValue())) {
                                removeSize++;
                            }
        ...
        if (holder.persistent.asyn()) {
            ...
            if (asyncDBEntity == null) {
                ...
                try {
                    ...
                    LOGGER.error(
                            MessageFormat.format("#LoaclAsynBDObjectPool#提交状态为{0}对象{1},进行{2}操作异常", AsyncDBState.DELETED, object,
                                    AsyncDBState.INSERT),
                            e);
                }
                ...
                if (asyncDBEntity.isDelete()) {
                    try {
                        if (asyncDBEntity.mark(Operation.INSERT, object)) {
                            return this.syncDBExecutor.summit(asyncDBEntity);
                        }
    ...
    private boolean doOperation(String key, Object object, Operation operation, AsynDBClassHolder holder, boolean checkNull) {
        ...
        try {
            ...
            if (submit) {
                ...
                boolean result = this.syncDBExecutor.summit(asyncDBEntity);
