...
public abstract class MessageHeaderTest extends ProtocolTest {
    ...
    public abstract MessageHeader create(int id, int protocol, int code, long time, int toMessage, Object head);
    ...
    public void isHasHead() {
        assertTrue(REQUEST_HEAD.isHasHead());
        assertTrue(SUCCESS_RESPONSE_HEAD.isHasHead());
        assertFalse(FAIL_RESPONSE_HEAD.isHasHead());
        assertFalse(PUSH_HEAD.isHasHead());
    }

    @Test
    public void getHead() {
        assertEquals(REQUEST_HEAD.getHead(String.class), head);
        try {
            REQUEST_HEAD.getHead(Map.class);
            fail(StringAide.format("获取 head {} 类型错误没有抛出异常", Map.class));
        } catch (Exception e) {
            assertTrue(true);
        }
        assertEquals(SUCCESS_RESPONSE_HEAD.getHead(LIST_HEAD_TYPE), listHead);
        try {
            SUCCESS_RESPONSE_HEAD.getHead(MAP_HEAD_TYPE);
            fail(StringAide.format("获取 head {} 类型错误没有抛出异常", MAP_HEAD_TYPE));
        } catch (Exception e) {
            assertTrue(true);
        }
        assertNull(FAIL_RESPONSE_HEAD.getHead(String.class));
        assertNull(PUSH_HEAD.getHead(String.class));
    }
