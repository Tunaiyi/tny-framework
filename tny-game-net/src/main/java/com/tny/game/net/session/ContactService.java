...
public class ContactService {
    ...
    public ContactService(SessionKeeperManager sessionKeeperManager) {
        this.sessionKeeperManager = sessionKeeperManager;
    }

    /**
     * 获取contactType用户类型中与指定identify对应的Session <br>
     *
     * @param contactType 用户类型
     * @param identify    用户ID
     * @return 返回获取的session, 无session返回null
     */
    public Optional<Session> getSession(ContactType contactType, long identify) {
        Optional<SessionKeeper> sessionKeeperOpt = this.sessionKeeperManager.getKeeper(contactType);
        return sessionKeeperOpt.map(k -> k.getSession(identify));
    }
    ...
    public void pushByUid(ContactType contactType, long identify, Protocol protocol, ResultCode resultCode, Object body) {
        ...
        keeperOpt.ifPresent(k -> k.sendTo(identify, toPush(protocol, resultCode, body)));
    }
    ...
    public void push2User(Contact user, Protocol protocol, ResultCode resultCode, Object body) {
        Optional<SessionKeeper> keeperOpt = this.sessionKeeperManager.getKeeper(user.getContactType());
        keeperOpt.ifPresent(k -> k.sendTo(user.getContactId(), toPush(protocol, resultCode, body)));
    }
     ...
     */
    public int getSessionSize(ContactType contactType) {
        Optional<SessionKeeper> keeperOpt = this.sessionKeeperManager.getKeeper(contactType);
        return keeperOpt.map(SessionKeeper::size).orElse(0);
    }

    /**
     * @return 获取所有默认组用户session
     */
    public Set<Session> getAllSessions(ContactType contactType) {
        Optional<SessionKeeper> keeperOpt = this.sessionKeeperManager.getKeeper(contactType);
        Collection<Session> sessions = keeperOpt.map(k -> k.getAllSessions().values()).orElseGet(ImmutableList::of);
