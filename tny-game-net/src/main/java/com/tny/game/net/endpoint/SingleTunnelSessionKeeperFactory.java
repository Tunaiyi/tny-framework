 ...
 */
@Unit
public class SingleTunnelSessionKeeperFactory<UID> implements SessionKeeperFactory<UID, SessionKeeperSetting> {

    @Override
    public EndpointKeeper<UID, Session<UID>> createKeeper(String userType, SessionKeeperSetting setting) {
        return new SingleTunnelSessionKeeper<>(userType, setting);
    }
