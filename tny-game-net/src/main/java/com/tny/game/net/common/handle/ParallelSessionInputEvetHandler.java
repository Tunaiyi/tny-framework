 ...
 */
public class ParallelSessionInputEvetHandler<UID, S extends NetSession<UID>> implements SessionInputEventHandler<UID, S> {

    public static final Logger LOGGER = LoggerFactory.getLogger(ParallelSessionInputEvetHandler.class);

    private MessageDispatcher messageDispatcher;

    private MessageCommandExecutor commandExecutor;

    private final static AttrKey<AtomicBoolean> INPUT_HANDLE_STATE = AttrUtils.key(ParallelSessionInputEvetHandler.class, "INPUT_HANDLE_STATE");

    public ParallelSessionInputEvetHandler(MessageDispatcher messageDispatcher, MessageCommandExecutor commandExecutor) {
