...
public abstract class AbstractEndpoint<UID> extends AbstractNetter<UID> implements NetEndpoint<UID>, Endpoint<UID>  {
    ...
    public boolean receive(Tunnel<UID> tunnel, Message<UID> message) {
        ...
        if (filter != null && !filter.isCanHandler(message)) {
            ...
            if (mode == MessageMode.REQUEST) {
                this.sendAsyn(tunnel, MessageSubjectBuilder
    ...
    public SendContext<UID> sendAsyn(Tunnel<UID> tunnel, MessageSubject subject, MessageContext<UID> messageContext) {
        try {
            if (tunnel == null)
                tunnel = select(subject, messageContext);
            Message<UID> message = createMessage(subject, messageContext);
    ...
    public SendContext<UID> sendAsyn(MessageSubject subject, MessageContext<UID> messageContext) {
        return this.sendAsyn(null, subject, messageContext);
    }
    ...
    public SendContext<UID> sendSync(MessageSubject subject, MessageContext<UID> messageContext, long timeout) throws NetException {
        return this.sendSync(null, subject, messageContext, timeout);
    }
    ...
    private NetTunnel<UID> select(MessageSubject subject, MessageContext<UID> messageContext) throws SessionException {
        if (isClosed())
            throw new SessionException("Session [{}] 已经关闭, 无法发送", this);
        NetTunnel<UID> tunnel = selectTunnel(subject, messageContext);
        if (tunnel == null)
            throw new NullPointerException(format("{} is no tunnel", this));
        return tunnel;
    }

    protected abstract NetTunnel<UID> selectTunnel(MessageSubject subject, MessageContext<UID> messageContext);
