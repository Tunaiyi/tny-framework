...
public class NettyConnector<UID> extends NettyBootstrap implements ClientConnector<UID> {
    ...
    public NettyConnector(String name, ChannelMaker<Channel> channelMaker, NettyAppConfiguration appConfiguration) {
    ...
    Optional<Channel> connect(URL url, long connectTimeout) throws NetException {
        ...
        try {
            ...
            if (result && success) {
                return Optional.ofNullable(channelFuture.channel());
            }
            ...
            if (channelFuture.cause() != null) {
                ...
                throw new NetException(format("Connect url: {} failed. result: {} success: {}, connected: {}",
                        url, result, success, connected), channelFuture.cause());
            } else {
                channelFuture.cancel(true);
                throw new NetException(format("Connect url: {} timeout. result: {}, success: {}, connected: {}",
                        url, result, success, connected), channelFuture.cause());
            }
