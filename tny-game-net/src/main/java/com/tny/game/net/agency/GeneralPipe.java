...
import static com.tny.game.net.agency.PipeStatus.*;
...
public class GeneralPipe<UID> implements NetPipe<UID> {
    ...
    private final AgentDatagramTransmitter transmitter;
    ...
    public GeneralPipe(AgentDatagramTransmitter transmitter, NetBootstrapContext<UID> bootstrapContext) {
    ...
    public Tubule<UID> connectTubule(long tunnelId, String host, int port) throws PipeClosedException {
        ...
        synchronized (this) {
            ...
            if (this.tubuleMap.putIfAbsent(tunnelId, tubule) == null) {
                tubule.open();
                this.transmitter.write(new ConnectedDatagram(tubule.getId()), null);
            }
        }
        return tubule;
    }
    ...
    public void destroyTubule(NetTubule<UID> tubule) {
        if (this.tubuleMap.remove(tubule.getId(), tubule)) {
            ...
            this.transmitter.write(new DisconnectedDatagram(tubule.getId()), null);
        }
    }

    @Override
    public AgentDatagramTransmitter getTransmitter() {
        return this.transmitter;
    }

    protected void onClose() {
        AgentDatagramTransmitter transporter = this.transmitter;
