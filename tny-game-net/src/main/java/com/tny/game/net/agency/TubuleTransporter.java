...
public class TubuleTransporter<UID> implements Transporter<UID> {
    ...
    public WriteMessageFuture write(Message message, WriteMessagePromise promise) throws NetException {
        return this.transmitter.write(new MessageDatagram(this.tubule, message), promise);
    }
    ...
    public void write(MessagesCollector collector) {
        for (Message message : collector.collect()) {
            this.transmitter.write(new MessageDatagram(this.tubule, message), null);
        }
    }
    ...
    public void pong() {
        this.transmitter.write(HeartbeatDatagram.pong(this.tubule.getId()), null);
    }

    public void ping() {
        this.transmitter.write(HeartbeatDatagram.ping(this.tubule.getId()), null);
    }
