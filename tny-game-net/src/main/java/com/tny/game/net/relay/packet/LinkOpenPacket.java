...
public class LinkOpenPacket extends BaseLinkPacket<LinkOpenArguments> {
    ...
    public LinkOpenPacket(int id, LinkOpenArguments arguments) {
        super(id, RelayPacketType.LINK_OPEN, arguments);
    }

    public LinkOpenPacket(int id, LinkOpenArguments arguments, long time) {
        super(id, RelayPacketType.LINK_OPEN, time, arguments);
    }

    @Override
    protected String toPacketMessage() {
        return "Link # 打开连接 [ RelayTunnel(" + arguments.getKey() + "@" + arguments.getService() + "#" + arguments.getInstance() + ") ]";
    }
