...
import com.tny.game.net.session.event.SessionResendOrder;
 ...
 */
public abstract class CommonNetSession<UID, S extends CommonNetSession<UID, S>> implements NetSession<UID> {
    ...
    public CommonNetSession(SessionOutputEventHandler<UID, S> writer, SessionInputEventHandler<UID, S> reader, int cacheMessageSize) {
    ...
    public void sendMessage(Protocol protocol, MessageContent content) {
        ...
        try {
            synchronized (this) {
                ...
                this.outputEventQueue.add(new SessionSendEvent(message, SessionEventType.MESSAGE, content.getSentHandler(), content.getMessageFuture()));
                if (content.hasMessageFuture())
                    holder().putFuture(content.getMessageFuture());
    ...
    public void receiveMessage(Message<UID> message) {
        if (message instanceof NetMessage)
            ((NetMessage<UID>) message).setSession(this);
    ...
    private void resendMessage(Range<Integer> range) {
        this.outputEventQueue.add(new SessionResendOrder(range));
    }
