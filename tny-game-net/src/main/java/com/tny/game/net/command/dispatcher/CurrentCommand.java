 ...
 * <br>
 */
public class CurrentCommand<UID> {
    ...
    private static ThreadLocal<CurrentCommand> local = new ThreadLocal<CurrentCommand>();
    ...
    private CurrentCommand(UID userID, int protocol, Thread thread) {
        this.userID = userID;
     ...
     * @return
     */
    public static <ID> CurrentCommand<ID> getCurrent() {
        CurrentCommand info = local.get();
        if (info == null) {
            info = new CurrentCommand();
            info.protocol = 0;
            info.userID = -1;
            local.set(info);
        }
