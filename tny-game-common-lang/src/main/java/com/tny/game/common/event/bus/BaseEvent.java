...
public abstract class BaseEvent<D, E extends BaseEvent<D, E>> implements Event<D, E> {
    ...
    protected Supplier<List<D>> factory;

    private volatile List<D> listeners;
    ...
    protected BaseEvent(Supplier<List<D>> factory) {
        this.factory = factory;
    }

    protected Optional<List<D>> listenerOpt() {
        return Optional.ofNullable(listeners);
    }

    protected List<D> readOnlyListeners() {
        return listeners == null ? List.of() : listeners;
    }

    protected List<D> loadListeners() {
        if (listeners != null) {
            return listeners;
        }
        synchronized (this) {
