...
public class SpringToCacheClassHolderAndLinkHandlerFactory
    ...
    private final List<String> scannerPath;
    ...
    private void loadClass() {
        this.task = ForkJoinPool.commonPool()
                .submit(() -> {
                    ClassSelector selector = ClassSelector.create(AnnotationClassFilter.ofInclude(ToCache.class));
                    ClassScanner.instance()
                            .addSelector(selector)
                            .scan(this.scannerPath.toArray(new String[0]));
    ...
    public void prepareStart() throws Exception {
        Map<String, CacheTrigger<?, ?, ?>> triggerMap = as(this.applicationContext.getBeansOfType(CacheTrigger.class, true, true));
        ...
        try {
            ...
            LOGGER.info("开始初始化 ToCacheClassHolder 完成! 耗时 {} ms", RunChecker.end(this.getClass()).costMillisTime());
        } catch (Throwable e) {
