...
public class ImportEndpointEventHandlerBeanDefinitionRegistrar extends BaseBeanDefinitionRegistrar {
    ...
    protected void loadBeanDefinition(String name, BeanDefinitionRegistry registry) {
        ...
        String handlerClassName = this.environment.getProperty(key(keyHead, CLASS_NODE), setting.getEventHandlerClass());
        String handlerName = getBeanName(name, EndpointEventsBoxHandler.class);
        Class<EndpointEventsBoxHandler> handlerClass = as(ExeAide.callUnchecked(() -> Class.forName(handlerClassName)).orElse(null));
