...
public abstract class BaseCipher implements AnyCipher {
    ...
    private void init() throws Exception {
        this.createCipher(Cipher.ENCRYPT_MODE, transform);
        this.createCipher(Cipher.DECRYPT_MODE, transform);
    ...
    public byte[] encrypt(byte[] data) throws Exception {
        Cipher cipher = encryptCipher.get().getCipher();
        return cipher.doFinal(data);
    }

    @Override
    public byte[] decrypt(byte[] encrypted) throws Exception {
        Cipher cipher = decryptCipher.get().getCipher();
