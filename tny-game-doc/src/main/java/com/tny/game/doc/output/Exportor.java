...
//    private static final Logger LOGGER = LoggerFactory.getLogger("Exporter");
    ...
    public Exportor clean() throws IOException {
        if (this.baseDir.exists()) {
            System.out.println(format("正在清空 [{}] 文件夹......", this.baseDir));
            deleteDirectory(this.baseDir);
            System.out.println(format("清空 [{}] 文件夹完成", this.baseDir));
        }
        return this;
    }
    ...
    public void exportAll(OutputType type) throws IOException {
        ...
        if (tempSet.isEmpty()) {
            System.out.println(format("{}包下未找到符合的类", StringUtils.join(basePackage, ";")));
            return;
        }
        ...
        for (Class<?> clazz : tempSet) {
            try {
                ...
                try (FileOutputStream output = openOutputStream(outputFile);
                    ...
                    System.out.println(format("正在导出 {} ......", outputFile.getAbsoluteFile()));
                    IOUtils.write(exporter.getHead(), writer);
                    IOUtils.write(headMessage, writer);
                    IOUtils.write(exporter.output(table), writer);
                    System.out.println(format("导出 {} 完成", outputFile.getAbsoluteFile()));
                    count++;
                }
            } catch (Throwable e) {
                e.printStackTrace();
//                LOGGER.error("{} 类导出异常", clazz, e);
        ...
        System.out.println(format("{} 包中,一共导出 {} 个文件到 {} 文件夹下", basePackage, count, baseDir.getAbsoluteFile()));
    }
