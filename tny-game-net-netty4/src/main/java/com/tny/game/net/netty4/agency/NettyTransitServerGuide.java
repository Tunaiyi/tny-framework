...
public class NettyTransitServerGuide extends NettyBootstrap<NettyServerBootstrapSetting> implements ServerGuide {
    ...
    public boolean close() {
        ...
        this.channels.forEach((address, channel) -> {
            try {
                NettyTransitServerGuide.LOG.info("#NettyTransitServer [ {} ] | Channel {} 关闭中......", this.setting.getName(), channel);
                channel.disconnect();
                NettyTransitServerGuide.LOG.info("#NettyTransitServer [ {} ] | Channel {} 关闭完成", this.setting.getName(), channel);
            } catch (Throwable e) {
                NettyTransitServerGuide.LOG.error("#NettyTransitServer [ {} ] | Channel {} 关闭异常!!!", this.setting.getName(), channel, e);
                LOG.error("NettyTransitServer [ {} ] | {} close exception", this.setting.getName(), address, e);
            }
        });
        parentGroup.shutdownGracefully();
        childGroup.shutdownGracefully();
        NettyTransitServerGuide.this.fireServerClosed();
        NettyTransitServerGuide.LOG.info("#NettyTransitServer [ {} ] | 服务器已关闭!!!", this.setting.getName());
        return true;
    }
    ...
    private ServerBootstrap bootstrap() {
        ...
        synchronized (this) {
            ...
            NettyTransitDatagramHandler datagramHandler = new NettyTransitDatagramHandler();
            ...
            this.bootstrap.childHandler(new ChannelInitializer<Channel>() {
                ...
                protected void initChannel(Channel channel) throws Exception {
                    ChannelMaker<Channel> maker = NettyTransitServerGuide.this.channelMaker;
                    if (maker != null) {
