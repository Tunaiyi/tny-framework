...
public class Trades {
	...
	public static Trade cost(Action action, long id, StuffModel itemModel, Number number, AlterType alertType,
			DemandParamEntry<?>... params) {
		return trade(action, TradeType.COST, id, itemModel, number, alertType, params);
	}

	public static Trade cost(Action action, long id, StuffModel itemModel, Number number, AlterType alertType,
			Map<DemandParam, Object> paramMap) {
		return trade(action, TradeType.COST, id, itemModel, number, alertType, paramMap);
	}

	public static Trade cost(Action action, StuffModel itemModel, Number number, AlterType alertType,
			DemandParamEntry<?>... params) {
		return trade(action, TradeType.COST, new SimpleTradeItem<>(0, itemModel, number, alertType, true, demandParamMap(params)));
	}

	public static Trade cost(Action action, StuffModel itemModel, Number number, AlterType alertType,
			Map<DemandParam, Object> paramMap) {
		return trade(action, TradeType.COST, new SimpleTradeItem<>(0, itemModel, number, alertType, true, paramMap));
	}

	public static Trade cost(Action action, TradeItem<?> item, Number number) {
		return trade(action, TradeType.COST, new SimpleTradeItem<>(item, number));
	}

	public static Trade cost(Action action, TradeItem<?>... items) {
		return trade(action, TradeType.COST, items);
	}
